...
export default function Stats() {
  ...
  if (!unlocked) {
    return (
      <section className="max-w-md mx-auto px-6 py-32">
        ...
        <p className="mt-2 text-sm font-light text-[#475569]">Private dashboard. Enter passcode.</p>
        <form
        ...
        >
          <input
            ...
            autoFocus
            className="w-full bg-transparent border-b border-[#475569]/40 py-2 text-sm font-light focus:outline-none focus:border-[#1A1A1A]"
          />
          {err && <p className="text-xs text-red-600">{err}</p>}
          <button
            type="submit"
            disabled={loading || !passcode}
            className="w-full border border-[#1A1A1A] px-5 py-2 text-sm font-medium hover:bg-[#1A1A1A] hover:text-[#F7F5F2] transition-colors disabled:opacity-40"
          >
  ...
  if (!data) {
    return <section className="max-w-6xl mx-auto px-6 py-20 text-sm font-light text-[#475569]">Loading…</section>;
  }
    ...
    <section className="max-w-6xl mx-auto px-6 py-16">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-2 text-sm font-light text-[#475569]">Live · last {data.days} days</p>
        </div>
          ...
          {[7, 30, 90, 365].map((d) => (
            <button
              ...
              className={`text-xs uppercase tracking-widest px-3 py-1.5 border transition-colors ${
                days === d ? "border-[#1A1A1A] bg-[#1A1A1A] text-[#F7F5F2]" : "border-[#1A1A1A]/30 hover:border-[#1A1A1A]"
              }`}
            >
              {d}d
            </button>
      ...
      <div className="mt-10 border border-[#1A1A1A]/15 p-5">
        <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Daily views</h3>
        <div className="flex items-end gap-1 mt-5 h-40">
          {data.daily.map((d) => (
            <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group">
              <div
                className="w-full bg-[#1A1A1A] group-hover:bg-[#475569] transition-colors"
        ...
        <div className="flex justify-between mt-3 text-[10px] font-light text-[#475569]">
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <div className="border border-[#1A1A1A]/15 p-5">
          <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Top pages</h3>
          <ul className="mt-4 space-y-3">
            {data.topPaths.map((p) => (
              <li key={p.path}>
                <div className="flex justify-between text-sm font-light"><span className="truncate">{p.path}</span><span className="tabular-nums text-[#475569]">{p.count}</span></div>
        ...
        <div className="border border-[#1A1A1A]/15 p-5">
          <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Top referrers</h3>
          <ul className="mt-4 space-y-3">
            {data.topReferrers.map((r) => (
              <li key={r.ref}>
                <div className="flex justify-between text-sm font-light"><span className="truncate">{r.ref}</span><span className="tabular-nums text-[#475569]">{r.count}</span></div>
      ...
      <div className="grid md:grid-cols-3 gap-4 mt-4">
        <div className="border border-[#1A1A1A]/15 p-5">
          <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Devices</h3>
          <ul className="mt-4 space-y-3">
            {data.devices.map((d) => (
              <li key={d.name}>
                <div className="flex justify-between text-sm font-light"><span>{d.name}</span><span className="tabular-nums text-[#475569]">{d.count}</span></div>
        ...
        <div className="border border-[#1A1A1A]/15 p-5">
          <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Browsers</h3>
          <ul className="mt-4 space-y-3">
            {data.browsers.map((b) => (
              <li key={b.name}>
                <div className="flex justify-between text-sm font-light"><span>{b.name}</span><span className="tabular-nums text-[#475569]">{b.count}</span></div>
        ...
        <div className="border border-[#1A1A1A]/15 p-5">
          <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Hour of day</h3>
          ...
          <div className="flex justify-between mt-2 text-[10px] font-light text-[#475569]"><span>0</span><span>12</span><span>23</span></div>
        </div>
      </div>

      {/* Messages */}
      <div className="mt-10">
        <h3 className="text-xs uppercase tracking-widest font-light text-[#475569]">Contact messages ({data.messages.length})</h3>
        <ul className="mt-4 space-y-3">
          {data.messages.length === 0 && <li className="text-sm font-light text-[#475569]">No messages yet.</li>}
          {data.messages.map((m) => (
            <li key={m.id} className="border border-[#1A1A1A]/15 p-4">
              <div className="flex justify-between items-start gap-4">
                <div>
                  <div className="text-sm font-medium">{m.name} <span className="text-[#475569] font-light">· {m.subject}</span></div>
                  <a href={`mailto:${m.email}`} className="text-xs text-[#475569] hover:text-[#1A1A1A]">{m.email}</a>
                </div>
                <span className="text-[10px] uppercase tracking-widest font-light text-[#475569] whitespace-nowrap">{new Date(m.created_at).toLocaleString()}</span>
              </div>
      ...
      <div className="mt-10">
        ...
        <ul className="mt-4 space-y-3">
          {data.guestbook.length === 0 && <li className="text-sm font-light text-[#475569]">No entries yet.</li>}
            ...
            <li key={g.id} className="border border-[#1A1A1A]/15 p-4">
              <div className="flex justify-between items-start gap-4">
                <div>
                  ...
                  {g.email && <span className="text-xs text-[#475569]">{g.email}</span>}
                </div>
                <span className="text-[10px] uppercase tracking-widest font-light text-[#475569] whitespace-nowrap">{new Date(g.created_at).toLocaleString()}</span>
              </div>
      ...
      <details className="mt-10 border border-[#1A1A1A]/15 p-5">
        <summary className="text-xs uppercase tracking-widest font-light text-[#475569] cursor-pointer">Recent raw views ({data.recentViews.length})</summary>
        <ul className="mt-4 space-y-1 text-xs font-light font-mono">
          {data.recentViews.map((v, i) => (
            <li key={i} className="flex justify-between gap-4 border-b border-[#1A1A1A]/10 py-1">
              <span>{v.path}</span>
              <span className="text-[#475569] truncate max-w-xs">{v.referrer || "direct"}</span>
              <span className="text-[#475569] whitespace-nowrap">{new Date(v.created_at).toLocaleString()}</span>
            </li>
          ))}
        </ul>
      </details>
      ...
      <div className="mt-12 flex justify-end">
        <button
          ...
          className="text-xs uppercase tracking-widest text-[#475569] hover:text-[#1A1A1A]"
        >
          Lock
        </button>
      </div>
